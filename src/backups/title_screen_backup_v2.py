...
class TitleScreen(Scene):
    ...
    def __init__(self, manager, assets, input_mgr):
        ...
        # Shifted slightly upwards and added Level Selector button in between
        self.start_btn_rect = pygame.Rect(LOGICAL_WIDTH // 2 - 220, LOGICAL_HEIGHT - 340, 440, 64)
        self.level_sel_rect = pygame.Rect(LOGICAL_WIDTH // 2 - 220, LOGICAL_HEIGHT - 260, 440, 64)
        self.exit_btn_rect = pygame.Rect(LOGICAL_WIDTH // 2 - 220, LOGICAL_HEIGHT - 180, 440, 64)
    ...
    def on_enter(self, **kwargs):
        ...
        # Load or initialize the starting level
    ...
    def handle_events(self, events, input_mgr):
        ...
        if input_mgr.just_pressed[input_mgr.LEFT]:
            self.starting_level = max(1, self.starting_level - 1)
        ...
        if input_mgr.just_pressed[input_mgr.RIGHT]:
            self.starting_level = min(4, self.starting_level + 1)
            self.assets.play_sound("jump.wav", volume=0.15)
            self.manager.shared["starting_level"] = self.starting_level
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN and self.elapsed > 0.8:
                ...
                if self.start_btn_rect.collidepoint(mx, my):
                    self.assets.play_sound("level_complete.wav", volume=0.35)
                    self.manager.switch_to(SCENE_CHARACTER_SELECT)
                elif self.level_sel_rect.collidepoint(mx, my):
                    # Clicking left 25% decrements, right 25% increments, center cycles
                    if mx < self.level_sel_rect.left + 110:
                        self.starting_level = max(1, self.starting_level - 1)
                    elif mx > self.level_sel_rect.right - 110:
                        self.starting_level = min(4, self.starting_level + 1)
                    else:
                        self.starting_level = self.starting_level + 1
                        if self.starting_level > 4:
                            self.starting_level = 1
                    self.assets.play_sound("jump.wav", volume=0.15)
                    self.manager.shared["starting_level"] = self.starting_level
    ...
    def update(self, dt):
        ...
        new_hover_level = self.level_sel_rect.collidepoint(mx, my)
        if new_hover_level and not self.hover_level:
            self.assets.play_sound("jump.wav", volume=0.12)
        self.hover_level = new_hover_level
    ...
    def draw(self, surface):
        ...
        level_name = LEVEL_NAMES.get(self.starting_level, "???")
        level_text = f"STARTING LEVEL: Level {self.starting_level} ({level_name})"

        buttons = [
            {"rect": self.start_btn_rect, "hover": self.hover_start, "text": "START PILGRIMAGE", "is_selector": False},
            {"rect": self.level_sel_rect, "hover": self.hover_level, "text": level_text, "is_selector": True},
            {"rect": self.exit_btn_rect, "hover": self.hover_exit, "text": "EXIT PILGRIMAGE", "is_selector": False}
        ]
        ...
        for btn in buttons:
            ...
            if btn["is_selector"]:
                # Draw elegant left/right visual click zones on the selector button
                arrow_color = COLOR_GOLD_BRIGHT if hover else COLOR_GOLD
                # Draw left arrow indicator inside button
                arr_l = self.font_body.render("◄", True, arrow_color)
                btn_surf.blit(arr_l, arr_l.get_rect(midleft=(20, draw_rect.height // 2)))
                # Draw right arrow indicator inside button
                arr_r = self.font_body.render("►", True, arrow_color)
                btn_surf.blit(arr_r, arr_r.get_rect(midright=(draw_rect.width - 20, draw_rect.height // 2)))
                
                # Re-blit updated button surface
                surface.blit(btn_surf, draw_rect.topleft)
            ...
            if hover:
                ...
                arr_l_ind = self.font_body.render("►", True, (*COLOR_GOLD_BRIGHT[:3], glow_alpha))
                arr_r_ind = self.font_body.render("◄", True, (*COLOR_GOLD_BRIGHT[:3], glow_alpha))
                surface.blit(arr_l_ind, arr_l_ind.get_rect(midright=(draw_rect.left - 15, draw_rect.centery)))
                surface.blit(arr_r_ind, arr_r_ind.get_rect(midleft=(draw_rect.right + 15, draw_rect.centery)))
