...
class BoxSystem:
    ...
    def _place_boxes(self, platforms):
        """Place boxes on top of platforms. For Level 1, the GOAL box is pinned
        to the temple door platform (last platform, far right)."""
        ...
        # For Level 1: pin the Temple Key (GOAL) box to the temple door platform
        # The temple door platform is the last platform appended in _build_level1
        if self.level == 1:
            temple_door_plat = platforms[-1]  # Last platform = temple door (x~1740)
            goal_def = next((d for d in shuffled if d["cat"] == CAT_GOAL), None)
            other_defs = [d for d in shuffled if d["cat"] != CAT_GOAL]

            # Place goal box on temple door platform
            if goal_def:
                bx = temple_door_plat.centerx - Box.SIZE // 2
                by = temple_door_plat.y - Box.SIZE - 5
                self.boxes.append(Box(bx, by, goal_def))

            # Place remaining boxes randomly on other platforms (exclude temple door)
            remaining_plats = [p for p in available if p != temple_door_plat]
            random.shuffle(remaining_plats)
            for i, item_def in enumerate(other_defs):
                plat = remaining_plats[i % len(remaining_plats)]
                bx = plat.x + random.randint(10, max(10, plat.width - Box.SIZE - 10))
                by = plat.y - Box.SIZE - 5
                self.boxes.append(Box(bx, by, item_def))
        else:
            # All other levels: fully random placement
