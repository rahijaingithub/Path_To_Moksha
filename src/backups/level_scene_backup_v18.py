...
class LevelScene(Scene):
    ...
    def handle_events(self, events, input_mgr):
        ...
        # ── Level Complete / To Be Continued: wait for player to press Continue ──
    ...
    def update(self, dt):
        ...
        if not self.player.frozen:
            ...
            if self.player.can_fly:
                # 8-way responsive flying/floating controls
                target_vy = 0
                ...
                # Smooth vertical acceleration (just like horizontal)
                self.player.vy += (target_vy - self.player.vy) * 15 * dt
    ...
    def _advance_level(self):
        ...
        # Task 2: For Level 2 in kid/standard mode, levels 3-4 are in development
        if self.level == 2 and self.game_mode in ("kid", "standard"):
            self.to_be_continued = True
            return   # stay on level_complete=True, draw will show "To be continued"
        if next_level > 4:
    ...
    def draw(self, surface):
        ...
        if self.level_complete:
            ...
            if self.to_be_continued:
                # ── "To be continued" ending for kid/standard after Level 2 ──
                tbc_surf = self.font_title.render("To be continued...", True, COLOR_SAFFRON)
                surface.blit(tbc_surf, tbc_surf.get_rect(center=(LOGICAL_WIDTH // 2, band_y + band_h + 55)))
                dev_note = self.font_body.render("Levels 3 & 4 are coming soon!", True, COLOR_GOLD_DIM)
                surface.blit(dev_note, dev_note.get_rect(center=(LOGICAL_WIDTH // 2, band_y + band_h + 110)))
                # Return to main menu button
                mm_surf = self.font_body.render("▶  RETURN TO MAIN MENU", True, COLOR_WHITE)
                mm_r = mm_surf.get_rect(center=(LOGICAL_WIDTH // 2, band_y + band_h + 165))
                btn_bg2 = pygame.Surface((mm_r.width + 60, mm_r.height + 24), pygame.SRCALPHA)
                btn_alpha2 = 150 + int(60 * math.sin(self.elapsed * 4))
                pygame.draw.rect(btn_bg2, (30, 20, 0, btn_alpha2), btn_bg2.get_rect(), border_radius=14)
                pygame.draw.rect(btn_bg2, COLOR_SAFFRON, btn_bg2.get_rect(), width=3, border_radius=14)
                surface.blit(btn_bg2, (mm_r.x - 30, mm_r.y - 12))
                surface.blit(mm_surf, mm_r)
                hint_c = self.font_small.render("Press ENTER / SPACE / click to return to main menu", True, COLOR_GOLD_DIM)
                surface.blit(hint_c, hint_c.get_rect(center=(LOGICAL_WIDTH // 2, mm_r.bottom + 20)))
            else:
