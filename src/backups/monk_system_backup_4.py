...
class Monk:
    ...
    def draw(self, surface, font_body, font_small):
        ...
        # Head (centered horizontally, y-pos around 20% of height)
        head_x = W // 2
        head_y = int(H * 0.2)
        head_r = int(H * 0.16)
        ...
        # 1. Picchi (Peacock Feather Whisk) - Rested on the ground on the left
        #    Handle on top, feather fan on bottom (near ground)
        picchi_x = int(W * 0.12)
        picchi_w = int(W * 0.2)
        picchi_h = int(H * 0.25)
        picchi_cx = picchi_x + picchi_w // 2
        # Handle extending upward from the feather
        pygame.draw.line(monk_surf, WOOD_BROWN, (picchi_cx, H - picchi_h - int(picchi_h * 0.5)), (picchi_cx, H - picchi_h), 3)
        # Feather fan at the bottom near the ground
