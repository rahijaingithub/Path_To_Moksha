...
class LevelScene(Scene):
    ...
    def handle_events(self, events, input_mgr):
        ...
        if self.game_over:
            inp = input_mgr
            if inp.just_pressed[inp.LEFT] or inp.just_pressed[inp.RIGHT]:
                self.game_over_choice = 1 - self.game_over_choice  # toggle 0/1
            ...
            if inp.just_pressed[inp.ACTION] or inp.just_pressed[inp.JUMP]:
                if self.game_over_choice == 0:
                    ...
                    # Back to title / restart from start
    ...
    def draw(self, surface):
        ...
        # Standard mode: Initialize gradient texture lazily if it doesn't exist
        if self.game_mode == "standard" and (not hasattr(self, "platform_glow_texture") or self.platform_glow_texture is None):
        ...
        for plat in self.platforms:
            ...
            # Standard mode: draw a beautiful golden glow to the upper border of platforms
            if self.game_mode == "standard":
                # Scale the 1-pixel-wide gradient to the platform width
                glow_surf = pygame.transform.scale(self.platform_glow_texture, (plat.width, grad_ht))
                # Center the 40px tall glow vertically on the platform's top edge
                gameplay_surf.blit(glow_surf, (plat.left, plat.top - grad_ht))
        ...
        # ── Level Goal Text (Centered in bottom band, only when no box results message is active) ──
        if not (self.box_system.message_timer > 0 and self.box_system.message):
            if self.level == 1:
                if self.box_system.goal_found:
                    goal_text = "Goal: Unlock and enter the Temple Gate!"
                else:
                    goal_text = "Goal: Find the Temple Key!"
            else:
                goal_text = "Goal: Find the Akshat (Sacred Rice)!"
        ...
        # ── Game Over overlay ──
        if self.game_over:
            go_overlay = pygame.Surface((LOGICAL_WIDTH, LOGICAL_HEIGHT), pygame.SRCALPHA)
            go_overlay.fill((0, 0, 0, 190))
            surface.blit(go_overlay, (0, 0))

            # Title
            go_title = self.font_title.render("TIME'S UP!", True, COLOR_RED)
            surface.blit(go_title, go_title.get_rect(center=(LOGICAL_WIDTH // 2, LOGICAL_HEIGHT // 2 - 120)))

            sub = self.font_body.render("The path to Moksha requires more practice...", True, COLOR_CREAM)
            surface.blit(sub, sub.get_rect(center=(LOGICAL_WIDTH // 2, LOGICAL_HEIGHT // 2 - 60)))

            # Buttons
            btn_labels = ["↩  Restart Level", "⌂  Back to Start"]
            btn_colors = [COLOR_GOLD_BRIGHT, COLOR_WHITE]
            btn_y = LOGICAL_HEIGHT // 2 + 30
            btn_gap = 340
            for i, label in enumerate(btn_labels):
                cx = LOGICAL_WIDTH // 2 + (i - 0.5) * btn_gap * 1.0
                ...
                t_surf = self.font_body.render(label, True, btn_colors[i] if selected else COLOR_CREAM)
                btn_surf.blit(t_surf, t_surf.get_rect(center=(150, 35)))
                surface.blit(btn_surf, (cx - 150, btn_y))

            hint = self.font_small.render("◄ ► to choose   •   SPACE / ✦ to confirm", True, COLOR_GOLD_DIM)
            surface.blit(hint, hint.get_rect(center=(LOGICAL_WIDTH // 2, btn_y + 100)))
