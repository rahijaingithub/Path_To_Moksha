...
class BoxSystem:
    ...
    def draw_message(self, surface, font):
        """Draw the item message in the center of the bottom controls band."""
        if self.message_timer > 0 and self.message:
            alpha = min(255, int(self.message_timer * 200))
            msg_surf = font.render(self.message, True, self.message_color)
            msg_surf.set_alpha(alpha)
            
            # Position at the center of the bottom 80px controls band (Y: LOGICAL_HEIGHT - 40)
            center_y = LOGICAL_HEIGHT - 40
            r = msg_surf.get_rect(center=(surface.get_width() // 2, center_y))
            
            # Background plate to overlay on top of the bottom band
            bg = pygame.Surface((r.width + 40, r.height + 12), pygame.SRCALPHA)
            # Semi-transparent dark overlay for readability
            pygame.draw.rect(bg, (10, 8, 15, min(220, alpha)), bg.get_rect(), border_radius=6)
            # Subtle border colored with the item category's color
            border_col = (*self.message_color[:3], min(120, alpha))
            pygame.draw.rect(bg, border_col, bg.get_rect(), width=2, border_radius=6)
            
            surface.blit(bg, (r.x - 20, r.y - 6))
