...
class VictoryScene(Scene):
    ...
    def on_enter(self, **kwargs):
        ...
        # Score calculation: base 10,000 minus time penalty plus bonus points
        monk_dict = self.manager.shared.get("monk_correct", {})
        correct_count = sum(1 for v in monk_dict.values() if v is True)
        final_score = max(500, int(10000 - self.total_time * 15 + correct_count * 1500))
        self.manager.shared["final_score"] = final_score

        # Record profile score
        player_name = self.manager.shared.get("player_name", "Pilgrim")
        character = self.manager.shared.get("character", "boy")
        from profile_manager import ProfileManager
        ProfileManager().save_profile(player_name, character=character, score=final_score, level_reached=4)
        ...
        self.font_sub = self.assets.load_font(None, GAME_FONT_SIZE_SUBTITLE)
