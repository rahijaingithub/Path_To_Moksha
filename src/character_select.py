...
from settings import (
    LOGICAL_WIDTH, LOGICAL_HEIGHT, SCENE_LEVEL, SCENE_TRANSITION, SCENE_PLAYER_SELECT,
...
class CharacterSelect(Scene):
    ...
    def handle_events(self, events, input_mgr):
        ...
        if input_mgr.just_pressed[input_mgr.RIGHT] or input_mgr.just_pressed[input_mgr.MENU_RIGHT]:
            if self.selected != 1:
                self.selected = 1
                self.assets.play_sound("jump.wav", volume=0.12)

        if input_mgr.just_pressed[input_mgr.ACTION] or input_mgr.just_pressed[input_mgr.JUMP] or input_mgr.just_pressed[input_mgr.MENU_SELECT]:
    ...
    def _confirm_selection(self):
        ...
        char_choice = "boy" if self.selected == 0 else "girl"
        self.manager.shared["character"] = char_choice

        # Save profile character choice persistently!
        player_name = self.manager.shared.get("player_name", "Pilgrim")
        from profile_manager import ProfileManager
        pm = ProfileManager()
        prof = pm.get_profile(player_name)
        score = prof.get("high_score", 0) if prof else 0
        lvl = prof.get("highest_level", 1) if prof else 1
        pm.save_profile(player_name, character=char_choice, score=score, level_reached=lvl)
