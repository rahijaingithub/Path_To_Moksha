...
class TransitionScene(Scene):
    ...
    def __init__(self, manager, assets, input_mgr):
        ...
        self.elapsed = 0.0
        self.fade_alpha = 255
        self.auto_advance_timer = 5.0  # auto-advance after 5 seconds
        self.data = {}
        self.bg_image = None
    ...
    def draw(self, surface):
        ...
        glow_r = 260 + int(20 * math.sin(self.elapsed * 2))
        glow = pygame.Surface((glow_r * 2, glow_r * 2), pygame.SRCALPHA)
        pygame.draw.circle(glow, (255, 200, 60, 30),
                           (glow_r, glow_r), glow_r)
        surface.blit(glow, (LOGICAL_WIDTH // 2 - glow_r,
                            LOGICAL_HEIGHT // 2 - glow_r - 40))

        # Bhagwan / Temple image (centered)
        if self.bg_image:
            img_rect = self.bg_image.get_rect(
        ...
        surface.blit(title, title.get_rect(
            center=(LOGICAL_WIDTH // 2, LOGICAL_HEIGHT // 2 + 240)))

        # Subtitle
        sub = self.font_body.render(self.data["subtitle"], True, COLOR_SAFFRON)
        surface.blit(sub, sub.get_rect(
            center=(LOGICAL_WIDTH // 2, LOGICAL_HEIGHT // 2 + 280)))

        # Message
        msg = self.font_small.render(self.data["message"], True, COLOR_CREAM)
        surface.blit(msg, msg.get_rect(
            center=(LOGICAL_WIDTH // 2, LOGICAL_HEIGHT // 2 + 320)))

        # "Press ENTER to continue" pulse
        if self.elapsed > 1.5:
            alpha = int(128 + 127 * math.sin(self.elapsed * 3))
